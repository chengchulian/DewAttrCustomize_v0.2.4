...
public class ThirdPersonCharacter : MonoBehaviour
{
	...
	private void UpdateAnimator(Vector3 move)
	{
		...
		float jumpLeg = (float)((Mathf.Repeat(m_Animator.GetCurrentAnimatorStateInfo(0).normalizedTime + m_RunCycleLegOffset, 1f) < 0.5f) ? 1 : (-1)) * m_ForwardAmount;
		if (m_IsGrounded)
		{
	...
	private void ApplyExtraTurnRotation()
	{
		...
		base.transform.Rotate(0f, m_TurnAmount * turnSpeed * Time.deltaTime, 0f);
	}
	...
	public void OnAnimatorMove()
	{
		...
		{
			Vector3 moveForward = base.transform.forward * m_Animator.GetFloat("motionZ") * Time.deltaTime;
