...
public class FPS_Controller : MonoBehaviour
{
	...
	private void Update()
	{
		...
		if (mousePos.x < 0f || mousePos.x >= (float)Screen.width || mousePos.y < 0f || mousePos.y >= (float)Screen.height)
		{
			return;
		}
		...
		Vector3 moveDirection = base.transform.forward * inputVert + base.transform.right * inputHor;
		moveDirection *= speed;
		if (isGrounded)
		{
